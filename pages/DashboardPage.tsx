...
const DashboardPage: React.FC<DashboardPageProps> = ({ user }) => {
  ...
  const handleVideoUpload = async () => {
    ...
    if (fileSizeMB > 50) {
      alert(`O vídeo é muito grande (${fileSizeMB.toFixed(2)}MB). O tamanho máximo é 50MB. Por favor, comprima o vídeo ou use um arquivo menor.`);
      return;
    }
    ...
    try {
      ...
      const videoPath = `models/${user.modelId}/verification_${timestamp}.${fileExt}`;
      ...
      uploadTimeout = setTimeout(() => {
        ...
        setUploadProgress(0);
        alert('O upload está demorando muito. Verifique sua conexão e tente novamente com um vídeo menor ou melhor comprimido.');
      }, timeoutDuration);
      ...
      if (videoUrl) {
        ...
        if (error) {
          ...
          alert('Vídeo enviado, mas houve um erro ao salvar. Tente novamente.');
          setUploadingVideo(false);
          setUploadProgress(0);
          return;
        }
        ...
        alert('Vídeo de verificação enviado com sucesso!');
        setVideoFile(null);
        ...
        alert('Erro ao fazer upload do vídeo. Verifique sua conexão e tente novamente.');
        setUploadingVideo(false);
        setUploadProgress(0);
      }
    } catch (err: any) {
      ...
      if (err.message?.includes('Timeout')) {
        alert('O upload demorou muito e foi cancelado. Tente novamente com um vídeo menor ou melhor comprimido.');
      } else {
        alert(`Erro ao fazer upload: ${err.message || 'Erro desconhecido'}\n\nDica: Tente comprimir o vídeo ou verificar sua conexão.`);
      }
    }
  };
          ...
          {!modelData?.verification_video && (
            <div className="rounded-xl bg-[#1c2127] border border-white/5 p-4 sm:p-6 shadow-xl">
              ...
              >
                ...
                <h4 className="text-base sm:text-lg font-bold mb-1.5 sm:mb-2">Envie seu Vídeo</h4>
                <p className="text-xs sm:text-sm text-slate-500 max-w-sm mb-4 sm:mb-6">Grave um vídeo de 5s segurando seu documento ao lado do rosto em um ambiente bem iluminado.</p>
                <button 
                  type="button"
                  className="flex items-center gap-1.5 sm:gap-2 rounded-lg bg-blue-600 px-4 sm:px-6 py-2 sm:py-2.5 text-xs sm:text-sm font-bold text-white hover:bg-blue-500 transition-all"
                >
                  <span className="material-symbols-outlined text-base sm:text-lg">cloud_upload</span>
                  Selecionar Arquivo
                </button>
