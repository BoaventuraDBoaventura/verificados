...
const EditProfilePage: React.FC<EditProfilePageProps> = ({ user }) => {
  ...
  const handleGalleryImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      ...
      if (remainingSlots <= 0) {
        alert('Você já atingiu o limite de 3 imagens. Remova algumas antes de adicionar novas.');
      ...
      if (files.length > remainingSlots) {
        alert(`Você pode adicionar apenas mais ${remainingSlots} imagem(ns). ${files.length - remainingSlots} arquivo(s) foram ignorados.`);
      }
      
      const validFiles = filesToAdd.filter(f => f.size <= 5 * 1024 * 1024);
      if (validFiles.length !== filesToAdd.length) {
        alert('Algumas imagens excedem 5MB e foram ignoradas.');
      }
  ...
  const handlePreviewVideoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      ...
      if (remainingSlots <= 0) {
        alert('Você já atingiu o limite de 3 vídeos. Remova alguns antes de adicionar novos.');
        e.target.value = ''; // Limpar o input
        return;
      }
      ...
      const filesToAdd = files.slice(0, remainingSlots);
      if (files.length > remainingSlots) {
        alert(`Você pode adicionar apenas mais ${remainingSlots} vídeo(s). ${files.length - remainingSlots} arquivo(s) foram ignorados.`);
      }
      
      const validFiles = filesToAdd.filter(f => f.size <= 30 * 1024 * 1024);
      if (validFiles.length !== filesToAdd.length) {
        alert('Alguns vídeos excedem 30MB e foram ignorados.');
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user.modelId) {
      alert('Erro: ID do modelo não encontrado. Faça login novamente.');
      return;
    }
    ...
    try {
      ...
      const updateData: any = {
        ...
        height: formData.height ? formData.height.trim() : null,
        weight: formData.weight ? formData.weight.trim() : null,
        waist: formData.waist ? formData.waist.trim() : null,
        bust: formData.bust ? formData.bust.trim() : null,
        eyes: formData.eyes ? formData.eyes.trim() : null,
        hair: formData.hair ? formData.hair.trim() : null,
        shoe_size: formData.shoe_size ? formData.shoe_size.trim() : null
      };
      ...
      // Upload de nova foto de perfil se houver
      ...
      if (profileImageFile) {
        ...
        const profileImagePath = `models/${user.modelId}/profile_${timestamp}.${fileExt}`;
        ...
        if (profileImageUrl) {
          ...
          updateData.profile_image = profileImagePreview || formData.profile_image || null;
          // Mesmo se falhar, incrementar contador para não afetar progresso dos outros arquivos
          fileCounter = 1;
        }
      } else {
        // Se não há nova foto, manter a existente ou limpar se foi removida
        updateData.profile_image = profileImagePreview || formData.profile_image || null;
        console.log('ℹ️ Nenhuma nova foto selecionada, mantendo:', updateData.profile_image);
      }

      // Identificar imagens da galeria que foram removidas (estavam no formData mas não estão mais nos previews)
      const originalGalleryUrls = Array.isArray(formData.gallery_images) ? formData.gallery_images : [];
      const currentGalleryUrls = galleryImagePreviews.filter(p => !p.startsWith('data:'));
      ...
      // Incrementar contador se foto de perfil foi enviada
      if (profileImageFile) {
        fileCounter = 1;
      }
      ...
      for (let i = 0; i < galleryImagePreviews.length; i++) {
        ...
        if (preview.startsWith('data:')) {
          if (newFileIndex < galleryImageFiles.length) {
            ...
            const galleryImagePath = `models/${user.modelId}/gallery_${timestamp}_${i}.${fileExt}`;
      ...
      for (let i = 0; i < previewVideoFiles.length; i++) {
        ...
        const timestamp = Date.now();
        const fileExt = file.name.split('.').pop();
        const videoPath = `models/${user.modelId}/video_${timestamp}_${i}.${fileExt}`;
        ...
        <section className="rounded-2xl bg-[#1c2127] p-4 md:p-6 border border-white/5 shadow-xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Calçado', name: 'shoe_size', type: 'text', placeholder: '37' },
            ].map(field => (
